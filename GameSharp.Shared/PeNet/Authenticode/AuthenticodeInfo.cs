...
// a.	http://www.cs.auckland.ac.nz/~pgut001/pubs/authenticode.txt
{
    public class AuthenticodeInfo
    {
        ...
        private X509Certificate2 GetSigningCertificate()
        {
            ...
            byte[] pkcs7 = _peFile.WinCertificate.bCertificate;
        ...
        private bool VerifyHash()
        {
            ...
            IEnumerable<byte> hash = ComputeAuthenticodeHashFromPeFile(hashAlgorithm);
        ...
        private byte[] GetSignedHash()
        {
            ...
            SignedData sd = new SignedData(_contentInfo.Content);
            if (sd.ContentInfo.ContentType != "1.3.6.1.4.1.311.2.1.4") // 1.3.6.1.4.1.311.2.1.4 = OID for Microsoft Crypto
            {
                return null;
            }

            Asn1Node spc = sd.ContentInfo.Content;
            Asn1OctetString signedHash = (Asn1OctetString)spc.Nodes[0].Nodes[1].Nodes[1];
        ...
        private IEnumerable<byte> ComputeAuthenticodeHashFromPeFile(HashAlgorithm hash)
        {
            ...
            int offset = Convert.ToInt32(_peFile.ImageNtHeaders.OptionalHeader.Offset) + 0x40;
            ...
            Structures.IMAGE_DATA_DIRECTORY certificateTable = _peFile.ImageNtHeaders.OptionalHeader.DataDirectory[4];
            ...
            int length = Convert.ToInt32(certificateTable.Offset) - offset;
            ...
            int sizeOfHeaders = Convert.ToInt32(_peFile.ImageNtHeaders.OptionalHeader.SizeOfHeaders);
            ...
            int fileSize = _peFile.Buff.Length;
            int sizeOfAttributeCertificateTable = Convert.ToInt32(certificateTable.Size);
